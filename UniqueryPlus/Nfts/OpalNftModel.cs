...
using Opal.NetApi.Generated.Model.pallet_evm.account;
...
using Opal.NetApi.Generated.Model.pallet_nonfungible;
...
{
    public record OpalNftFull : OpalNft
    {
        private SubstrateClientExt client;

        public IEnumerable<INftBase> Nests { get; set; } = new List<INftBase>();
        public OpalNftFull(SubstrateClientExt client) : base(client)
        {
            this.client = client;
        }
    }
    public record OpalNft : INftBase, INftTransferable, INftBurnable, INftNestable, INftBaseNestable
    {
        private SubstrateClientExt client;
        ...
        {
            var nfts = await OpalNftModel.GetNestedNftsByIdAsync(client, (uint)CollectionId, (uint)Id, null, token).ConfigureAwait(false);
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    ...
    public class OpalNftModel
    {
        ...
        internal static async Task<IEnumerable<INftBase>> GetNftsOwnedByAsync(SubstrateClientExt client, string owner, int limit, int offset, CancellationToken token)
        {
            ...
            var result = await opalSubqueryClient.GetNftsOwnedBy.ExecuteAsync(owner, limit, offset).ConfigureAwait(false);
            ...
            return result.Data.Tokens.Data?.Select(token => new OpalNft(client)
            {
        ...
        internal static async Task<RecursiveReturn<INftBase>> GetNftsByIdKeysAsync(SubstrateClientExt client, IEnumerable<string> idKeys, string lastKey, CancellationToken token)
        {
            ...
            var collectionDatas = await OpalCollectionModel.GetCollectionCollectionByCollectionIdKeysAsync(client, collectionIdKeys, token).ConfigureAwait(false);
            ...
            return new RecursiveReturn<INftBase>
            {
                ...
                }).Zip(collectionDatas, (OpalNft nft, Collection? collectionData) =>
                {
                    ...
                    return nft;
                }).Zip(nftMetadatas, (OpalNft nft, MetadataBase? metadata) =>
                {
                    // Should never be null
