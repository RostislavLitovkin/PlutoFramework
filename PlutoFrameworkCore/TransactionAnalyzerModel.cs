            ...
            Dictionary<string, Dictionary<AssetKey, Asset>>? existingCurrencyChanges = null)
        {
            ...
            foreach (var e in events)
            {
                IEnumerable<(string, AssetKey, BigInteger)> evaluated = e switch
                {
                    ...
                    ExtrinsicEvent { PalletName: "AssetsHolder", EventName: nameof(XcavatePaseo.NetApi.Generated.Model.pallet_assets_holder.pallet.Event.Held) } => [(e.Parameters[0].Value, (endpoint.Key, AssetPallet.AssetsReserved, BigInteger.Parse(e.Parameters[1].Value)), -BigInteger.Parse(e.Parameters[3].Value) * 1_000_000)],// Has to be * 10^6 on Xcavate blockchain
